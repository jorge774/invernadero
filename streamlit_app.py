...
a.metric("🌡️ Temp (°C)", f"{ultima['temperatura']:.1f}")
b.metric("💧 Humedad aire (%)", f"{ultima['humedad_aire']:.1f}")
c.metric("🌱 Humedad suelo (%)", f"{ultima['humedad_suelo']:.1f}")
d.metric("📈 Presión (kPa)", f"{ultima['presion']:.1f}")
e.metric("🟢 CO₂ (ppm)", f"{ultima['co2']:.0f}")
f.metric("💡 Lumenes", f"{ultima['lumenes']:.0f}")
